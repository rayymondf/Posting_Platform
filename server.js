...
app.post('/api/auth/guest-login', (req, res) => {
  req.login({ id: GUEST_ID, username: 'Guest' }, (err) => {
    if (err) return res.status(500).json({ error: 'Guest login failed' });
    res.json({ user: { id: GUEST_ID, username: 'Guest' }, message: 'Guest login successful' });
  });
});
...
app.post('/api/posts/:id/like', (req, res, next) => {
  ...
  try {
    ...
    res.json({ likes_count: parseInt(countResult.rows[0].count) });
  } catch (err) {
